/* This script makes Pheeno drive to different way points using a complementary filter
on the magnetometer, accelerometer, and encoders for feedback!
*/
...
float timeStep = 100; //Time step for the odometry update to operate at (10Hz)!
int count = 0;//iterator through the way points.
float desVel = 15;//Desired linear velocity of Pheeno in cm/s.
float northOffset = 12.9;//Difference between magnetic north and the global X axis
...
void loop() {
  while (calculateDistance(myRobot.botXPos,myRobot.botYPos,botXf[count%numWayPoints],botYf[count%numWayPoints]) > 3){
    ...
    myRobot.sensorFusionPositionUpdate(timeStep, northOffset); //Encoders used for state estimates.
  }
  count ++;
}
